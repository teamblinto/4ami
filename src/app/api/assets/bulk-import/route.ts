...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isMultipart) {
      ...
      const apiBaseUrl = process.env.NEXT_PUBLIC_API_BASE_URL || "https://4ami.morshadunnur.me/api/v1";
      if (apiBaseUrl.includes('ngrok')) {
        headers['ngrok-skip-browser-warning'] = 'true';
      }

      // Forward the FormData to the backend API
      const response = await fetch(getApiUrl('/assets/bulk-import'), {
        method: 'POST',
        headers,
        body: formData,
      });
    ...
    console.error('Assets bulk import proxy error:', error);
