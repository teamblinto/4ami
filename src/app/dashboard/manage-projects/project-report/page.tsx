...
export default function ProjectReportPage() {
  ...
  const noteSections = ['Overview', 'Key Findings', 'Market Conditions', 'Utilization Analysis', 'Recommendations', 'Next Steps'] as const;
  type NoteOptionValue = 'ai' | 'appraiser' | 'custom';
  type NoteStateRecord<T> = Record<(typeof noteSections)[number], T>;

  const defaultNoteSelection = noteSections.reduce<NoteStateRecord<NoteOptionValue>>((acc, section) => {
    acc[section] = 'ai';
    return acc;
  }, {} as NoteStateRecord<NoteOptionValue>);

  const defaultTextRecord = noteSections.reduce<NoteStateRecord<string>>((acc, section) => {
    acc[section] = '';
    return acc;
  }, {} as NoteStateRecord<string>);

  const [noteSelections, setNoteSelections] = useState<NoteStateRecord<NoteOptionValue>>(defaultNoteSelection);
  const [customNotes, setCustomNotes] = useState<NoteStateRecord<string>>(defaultTextRecord);
  const [noteErrors, setNoteErrors] = useState<NoteStateRecord<string>>(defaultTextRecord);
  ...
  const sharedNoteBody =
    'Comprehensive valuation assessment for Volvo A30G Articulating Dump truck with initial cost of $507,750, positioned in highly competitive articulated hauler market. Analysis indicates strong value retention potential with 72% first-year residual value, though subject to significant inflation pressures and utilization-dependent depreciation patterns. Equipment demonstrates above-average market positioning with optimal performance at 600-1,150 hours annually.';

  const noteOptions: { value: NoteOptionValue; title: string; body: string }[] = [
    { value: 'ai', title: 'Generated by AI', body: sharedNoteBody },
    { value: 'appraiser', title: 'Appraiser Note', body: sharedNoteBody },
    { value: 'custom', title: 'Custom Note', body: '' },
  ];

  const handleConfirmNote = (section: (typeof noteSections)[number]) => {
    const selectedOption = noteSelections[section];
    if (selectedOption === 'custom' && !customNotes[section].trim()) {
      setNoteErrors((prev) => ({ ...prev, [section]: 'Please enter a custom note before confirming.' }));
      return;
    }
    ...
    <div className="pt-4">
      <div className="space-y-4">
        {noteOptions.map((option) => {
          const isSelected = noteSelections[section] === option.value;
          return (
            <label
              key={option.value}
              className={`flex items-start gap-3 rounded-lg border p-4 cursor-pointer transition-colors ${isSelected ? 'border-red-500 bg-[#FFF4F4]' : 'border-gray-200 bg-white'}`}
            >
              <input
                type="radio"
                name={`${section}-note-option`}
                value={option.value}
                checked={isSelected}
                onChange={() => {
                  setNoteSelections((prev) => ({ ...prev, [section]: option.value }));
                  setNoteErrors((prev) => ({ ...prev, [section]: '' }));
                }}
                className="mt-1 h-5 w-5 accent-[#ED272C]"
              />
              <div className="flex-1">
                <p className="font-semibold text-gray-900">{option.title}</p>
                {option.value !== 'custom' ? (
                  <p className="text-sm text-gray-600 mt-2 leading-relaxed">{option.body}</p>
                ) : (
                  <>
                    {isSelected ? (
                      <textarea
                        value={customNotes[section]}
                        onChange={(e) => setCustomNotes((prev) => ({ ...prev, [section]: e.target.value }))}
                        rows={4}
                        placeholder="Add your note..."
                        className="mt-3 w-full rounded-md border border-gray-200 bg-white p-3 text-sm text-gray-700 focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500"
                      />
                    ) : (
                      <p className="text-sm text-gray-500 mt-2">Add a personalized note for the client.</p>
                    )}
                  </>
                )}
              </div>
      ...
      {noteErrors[section] && <p className="text-sm text-red-500 mt-3">{noteErrors[section]}</p>}
      <div className="flex justify-end mt-4">
        <button
          onClick={() => handleConfirmNote(section)}
          className="px-6 py-2 rounded-md bg-red-500 text-white hover:bg-red-600 cursor-pointer transition-colors"
        >
          Confirm Note
        </button>
      </div>
              ...
              <div key={section} className="bg-white rounded-lg shadow-sm">
                ...
                {expandedSections[section] && (
                  <div className="px-6 pb-4 border-t border-gray-100">
                    {noteSections.includes(section as (typeof noteSections)[number])
                      ? renderNoteSection(section as (typeof noteSections)[number])
                      : (
                        <div className="pt-4 text-gray-600">
                          <p>
                            Comprehensive valuation assessment for Volvo A30G Articulating Dump truck with initial cost of $507,750,
                            positioned in highly competitive articulated hauler market. Analysis indicates strong value retention
                            potential with 72% first-year residual value, though subject to significant inflation pressures and
                            utilization-dependent depreciation patterns. Equipment demonstrates above-average market positioning with
                            optimal performance at 600-1,150 hours annually. {section.toLowerCase()}.
                          </p>
                          <p className="mt-2">
                            Comprehensive valuation assessment for Volvo A30G Articulating Dump truck with initial cost of $507,750,
                            positioned in highly competitive articulated hauler market. Analysis indicates strong value retention
                            potential with 72% first-year residual value, though subject to significant inflation pressures and
                            utilization-dependent depreciation patterns. Equipment demonstrates above-average market positioning with
                            optimal performance at 600-1,150 hours annually.
                          </p>
                        </div>
                      )}
                  </div>
