...
export default function SendInvitationForm({ onInvitationSent }: SendInvitationFormProps) {
  ...
  const generateCode = () => {
    // More secure code generation with longer length and better randomness
    ...
    // Shuffle the result to randomize the position of required characters
    const shuffledResult = result.split('').sort(() => Math.random() - 0.5).join('');

    setInvitationCode(shuffledResult);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!invitationCode.trim()) {
        throw new Error('Please generate an invitation code before submitting');
      }
    ...
    <div className="bg-white  rounded-lg flex flex-col lg:flex-row w-full gap-8 overflow-hidden">
      ...
      <div className="w-full lg:w-3/5 p-6 min-w-0 flex-shrink">
        <div className="mb-[28px]">
          ...
          <p className="text-gray-500">Use this form to invite a business colleague or client to register and use the AMI website. Upon completion, a unique Invitation Code will be sent to your invitee. After they register, you will receive a notification email</p>
        </div>
